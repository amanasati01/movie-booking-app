...
import { locationInput, movieName, showTiming, theaterId } from "../atoms/Atoms";
...
export default function Card({ name }: { name: string }): JSX.Element {
  ...
  async function clickHandler(name: string, location: string, showTime: string, movieName: string) {
    try {
      const theater = await axios.post('http://localhost:3000/api/v1/tickets/generate-theater', {
        name,
        location,
        showTime,
        movieName,
      });
      console.log("Full theater data:", theater.data);
